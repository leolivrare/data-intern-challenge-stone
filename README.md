...
### Comentários sobre os resultados:
Vamos analisar os resultados obtidos por partes. A fim de encontar relações entre as transações fraudulentas e os dados do banco de dados, optei por fazer uma análise individual com cada dado que pode conter alguma relação. Dessa forma, vamos análisar cada análise:
#### Análise dos valores das transações:
Pode-se notar que os valores das transações fraudulentas são, em média, 7.73% mais altos do que das transações normais. Além disso, vemos que o valor máximo das transações fraudulentas é até menor que o máximo das transações normais, o que indica que não existe um valor que extrapola muito a média, puxando o valor da mesma para cima. Com isso, temos um valor mais fiel da média. Portanto, temos que os valores das fraudes são, em média, maiores que transações normais.
Por outro lado, temos um resultado bastante interessante acerca da mediana dos valores das transações. A mediana dos valores das transações fraudulentas é 17.2% maior do que a mediana dos valores das transações normais. Isso indica, e reforça, que as transações fraudulentas possuem um valor mais alto em relação às transações normais. Isso nos revela que transações de alto valor possuem maior chance de serem fraudes.
